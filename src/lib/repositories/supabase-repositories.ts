...
export const supabaseCategoryRepository = {
  async list(): Promise<Category[]> {
    return listJsonData<Category>("ecommerce_categories")
  },

  async getBySlug(slug: string): Promise<Category | null> {
    return getJsonDataBySlug<Category>("ecommerce_categories", slug)
  },

  async getById(id: string): Promise<Category | null> {
    return getJsonDataById<Category>("ecommerce_categories", id)
  },
...
export const supabaseProductRepository: ProductRepository = {
  async list(filters, sort, pagination) {
    const [products, categories] = await Promise.all([
      listJsonData<Product>("ecommerce_products", "created_at"),
  ...
  async getBySlug(slug) {
    const product = await getJsonDataBySlug<Product>("ecommerce_products", slug)
    return product?.status === "active" ? product : null
  },

  async getById(id) {
    return getJsonDataById<Product>("ecommerce_products", id)
  },

  async getFeatured(limit = 4) {
    const products = await listJsonData<Product>("ecommerce_products", "created_at")
    return products
  ...
  async getByCategory(categorySlug, pagination) {
    ...
    return paginate(
      products.filter(
        (product) =>
          product.status === "active" && product.categoryIds.includes(category.id)
      ),
      pagination
    )
  },
...
export const supabaseBrandRepository = {
  async list(): Promise<Brand[]> {
    return listJsonData<Brand>("ecommerce_brands")
  },

  async getBySlug(slug: string): Promise<Brand | null> {
    return getJsonDataBySlug<Brand>("ecommerce_brands", slug)
  },

  async getById(id: string): Promise<Brand | null> {
    return getJsonDataById<Brand>("ecommerce_brands", id)
  },
}

export const supabasePageRepository = {
  async list(): Promise<CmsPage[]> {
    const pages = await listJsonData<CmsPage>("ecommerce_pages", "published_at")
    return pages.sort(
      (a, b) =>
