...
import { Edit3, Image as ImageIcon, Loader2, MessageSquare, Star, Trash2 } from "lucide-react";
...
export function ReviewsSection({ productId }: { productId: string }) {
  ...
  function handleImageChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > maxFileSize) {
      toast.error("Dung lượng ảnh vượt quá giới hạn 2MB. Vui lòng chọn tệp nhỏ hơn.");
    ...
    <div className="mt-20 border-t border-[#E1DDD5]/80 bg-[#FCFAF2] py-16 selection:bg-[#EAE5D9]">
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        {totalReviews > 0 && (
          <div className="grid md:grid-cols-12 gap-8 bg-[#FAF5F2]/60 p-8 rounded-3xl border border-[#E1DDD5] mb-12 items-center">
            <div className="md:col-span-4 text-center md:border-r border-[#E1DDD5]/60 md:pr-8 py-4">
              <span className="font-serif text-6xl font-light text-black leading-none">{averageRating}</span>
              <div className="flex justify-center gap-1 my-3 text-[#FF9D00]">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star
                    key={i}
                    className={`h-5 w-5 ${i < Math.round(Number(averageRating)) ? "fill-[#FF9D00]" : "text-[#DCD6CC]"}`}
                  />
                ))}
              </div>
            ...
            <div className="md:col-span-8 space-y-2.5">
              {distribution.map((dist) => (
                <div key={dist.stars} className="flex items-center gap-4 text-xs font-mono text-[#786F66]">
                  ...
                  <span className="w-8 text-right font-medium text-black">{dist.count}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="grid gap-12 lg:grid-cols-5 items-start">
            ...
            {showForm && (
              ...
              >
                <div className="flex justify-between items-center border-b border-[#E1DDD5]/60 pb-3">
                  <h3 className="font-serif text-xl font-bold text-black">Gửi nhận xét từ bạn</h3>
                  <button
                    onClick={() => {
                      setShowForm(false);
                      handleRemoveImage();
                        ...
                        >
                          <Star
                            className={`h-7 w-7 transition-colors ${
                              star <= (hoveredStar ?? rating) ? "fill-[#FF9D00] text-[#FF9D00]" : "text-[#DCD6CC]"
                            }`}
                          />
                        </motion.button>
                      ))}
                      ...
                      <div className="relative border border-[#E1DDD5] rounded-2xl p-2 bg-white flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className="relative h-12 w-12 rounded-lg overflow-hidden border border-[#E1DDD5]">
                            <Image src={imagePreview} alt="Review preview" fill className="object-cover" />
                          </div>
          ...
          <div className={`${showForm ? "lg:col-span-3" : "lg:col-span-5"} space-y-6 w-full`}>
            {loading ? (
                  ...
                  {reviews.map((rev) => (
                    ...
                    >
                      <div className="flex items-center justify-between border-b border-[#E1DDD5]/40 pb-2">
                        <span className="font-serif text-sm font-semibold text-black">{rev.customer_name}</span>
                      ...
                      <div className="flex gap-0.5 text-[#FF9D00]">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <Star
                            key={i}
                            className={`h-3.5 w-3.5 ${i < rev.rating ? "fill-[#FF9D00]" : "text-[#DCD6CC]"}`}
                          />
                        ))}
                      </div>
